#!/usr/bin/env python3
"""
train_instr.py — Train an instruction-level GNN on Devign IR graphs.
...
Architecture:
  ...
  Linear(64 -> 1)              # binary classifier

Key differences from block-level train.py:
  - Node features are opcode vocab indices (int64), not float feature vectors
  - 3 edge relations (adds Global Context type) vs 2 in the block-level model
  - No z-score normalisation (integer indices, not floats)

Usage:
    python train_instr.py --epochs 30 --hidden 64   # baseline run
    python train_instr.py --epochs 60 --hidden 128  # extended run
"""
...
def main():
    ...
    for split in ["train", "valid", "test"]:
        if not (DATA / f"{split}_instr_graphs.pkl").exists():
            print(f"Missing data/{split}_instr_graphs.pkl -- run preprocess_instr.py first.")
            sys.exit(1)
