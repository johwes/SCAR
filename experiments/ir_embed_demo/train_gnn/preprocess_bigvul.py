#!/usr/bin/env python3
"""
preprocess_bigvul.py — Extract paired (vuln, fix) IR graphs from BigVul CSV.
"""
import argparse
import pickle
import random
...
from preprocess import compile_to_ir, ir_to_graph
...
def process_pair(args: tuple) -> dict | None:
    ...
    if fix_ir is None:
        return None
    vuln_g = ir_to_graph(vuln_ir)
    fix_g  = ir_to_graph(fix_ir)
...
def main():
    ...
    for split_name, split_pairs in splits.items():
        ...
        out = DATA / f"bigvul_{split_name}_pairs.pkl"
        with open(out, "wb") as f:
            pickle.dump(graphs, f)
        print(f"  Saved {len(graphs):,} graphs -> {out}")

    print("\nDone. Run train_triplet.py next.\n")
