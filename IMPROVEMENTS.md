...
### Clang Static Analyzer as a pluggable findings source
The LLVM toolchain already present in the `scar-ikos` container ships `clang --analyze` and `scan-build`. Running the analyzer as a parallel task alongside IKOS adds a complementary intra-procedural pass that catches null pointer dereferences, use-after-free, memory leaks, and API misuse patterns that IKOS's abstract interpretation does not model. The analyzer outputs `.plist` files (or SARIF on newer LLVM versions) which are trivially converted to `findings-clang-analyzer.json` using the same XML/JSON pattern as the cppcheck converter. Integration cost is low: the binary is already in the container, no new image is needed, and the conversion step is ~30 lines of Python.
...
### Semgrep as a pluggable findings source
Semgrep is a fast, pattern-based static analyzer with a large open-source C ruleset (`semgrep --config=p/c`) covering banned string functions, format string misuse, integer truncation, and common CWE patterns. It outputs JSON natively, runs in seconds to minutes on typical codebases, and integrates cleanly as a `scar-semgrep` Tekton task writing `findings-semgrep.json`. Unlike IKOS and cppcheck, Semgrep's rules are written in YAML and are straightforward to customize — teams can add project-specific patterns (e.g. internal unsafe API wrappers) without modifying SCAR itself. The signal-to-noise ratio on well-maintained community rulesets is high, making it a practical PR-gate scanner with minimal tuning. In an enterprise OSS stack, Semgrep is the lowest-friction entry point: `pip install semgrep`, one task YAML, done.

### Facebook Infer as a pluggable findings source
Infer (Meta, MIT license) uses bi-abduction — a form of compositional symbolic execution — to find resource leaks (memory, file handles, sockets), null pointer dereferences, and concurrency bugs (RacerD checker) that neither IKOS nor cppcheck model well. It runs against the actual build process (similar to `bear`) so it sees the real compiler flags and include paths. Integration follows Pattern B in the osscrs-scan architecture: Infer ships its own container image, the libCRS bridge is injected via `PYTHONPATH`, and the bug report JSON (`infer-out/report.json`) is converted to `findings-infer.json`. The main cost is a heavier container (JVM-based) and longer analysis time on large codebases. For C codebases with significant manual memory management — which are exactly SCAR's target — Infer's leak detection is meaningfully complementary to IKOS's overflow analysis.

### Git-diff scoping for PR gate deployment
Running a full scan on every PR is impractical for large codebases: the llm-scan alone takes 10–60 minutes depending on target size. Git-diff scoping cuts this to seconds for typical PRs by extracting the set of changed files (`git diff --name-only <base> HEAD`), expanding that set to include any file that `#include`s a changed header, and restricting the llm-scan and cppcheck tasks to that scope. IKOS still runs whole-program (its interprocedural analysis requires it), but the LLM token budget drops proportionally to the change size. Combined with the vulnerability lifecycle registry — which can serve cached verdicts for files confirmed clean in prior runs — this makes SCAR viable as a blocking PR check rather than a nightly batch job. Implementation requires git access inside the scan tasks and a `--diff-base` parameter threaded through the pipeline.
...
### Joern interprocedural taint analysis
Joern (Apache 2.0) is the open-source equivalent of CodeQL at the architectural level. It builds a Code Property Graph (CPG) — a unified representation combining AST, control-flow graph, and program dependence graph — across the entire codebase, then exposes a Scala DSL for writing taint-flow queries. A query can express "find all paths from a network read to a strcpy call that crosses a module boundary" and return the full chain. This fills the largest gap in the current SCAR scanner stack: IKOS and cppcheck are both intra-procedural; Joern operates across files natively. Integration is a dedicated `scar-joern` Tekton task (JVM container, ~2GB image) that runs Joern's CPG construction, executes a curated query set covering CWE-121/122/134/190, and writes `findings-joern.json`. The main costs are container size, JVM memory requirements (8GB+ for large targets), and the need to write and maintain Joern queries in the Scala DSL. For an enterprise OSS deployment targeting interprocedural vulnerability classes, Joern is the correct long-term investment.

### Cross-file data flow tracking
Untrusted remote inputs frequently traverse multiple structural definitions and files before hitting a dangerous execution sink (e.g., an network read inside `io.c` writes to a global struct passed to `parser.c`, which triggers a vulnerability inside `util.c`). Integrating full data-flow tracking graphs (via Joern, or static LLVM DataFlowSanitizers) is required to capture interprocedural taint trajectories. See the Joern entry above for the concrete OSS path.
...
* The enterprise OSS scanner stack in priority order: (1) Clang Static Analyzer — zero new infrastructure, already in container; (2) Semgrep — lowest friction, community rules, fast; (3) Infer — resource leaks and concurrency, fills a gap neither IKOS nor cppcheck covers; (4) Joern — interprocedural taint, the hardest and most impactful. All four feed `findings-<name>.json` and require no changes to the repair loop.
* SCAR's repair loop is designed to accept findings from any source — including proprietary enterprise tools like Fortify, Coverity, and CodeQL. Integrating these is a matter of writing a findings-format converter; the repair and triage stages are tool-agnostic.
